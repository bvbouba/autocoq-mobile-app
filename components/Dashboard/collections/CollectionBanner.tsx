    ...
    return <View style={styles.container}>
        ...
        <Carousel
            ...
            data={collection.products?.edges.map(edge => {
                const media = edge.node.thumbnail || undefined
                return {
                    url: media?.url,
                    alt: media?.alt,
                    name: edge.node.name,
            ...
            item ? (<Pressable onPress={() => router.push("products/details/" + item.id)}>
                <PaddedView style={styles.caroselItem} key={index}>
                    <View style={styles.carouselItemImageContainer}>
                        <Image source={{ uri: item.url }} resizeMode="contain" style={styles.carouselImage} />
                    </View>
                    <Text>{item.name}</Text>
                </PaddedView>
            </Pressable>): <View></View>
